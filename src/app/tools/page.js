...
function ToolsContent() {
  ...
  const checkAuthentication = async () => {
    ...
    const deviceId = localStorage.getItem('email');
    const userEmail = localStorage.getItem('email');
    const userStr = localStorage.getItem('user');
    const adminMode = localStorage.getItem('adminMode');

    if (adminMode === 'true') {
      // Admin mode removed - redirect to homepage
      return;
    }
    ...
    // User is authenticated, proceed with optional data fetching
    try {
      // Try to get premium status and trials, but don't fail if endpoint doesn't exist
      if (userEmail) {
        ...
        if (statusResponse && statusResponse.ok) {
          ...
          setHasPremium(statusData.hasPremium);
          setNoOfTrails(statusData.noOfTrails || 5);
        }
      }
    } catch (error) {
    ...
    // Mark auth as checked - allow page to render
    setIsAuthChecked(true);
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.email !== userEmail) {
          console.warn('Session user mismatch, clearing cache');
          localStorage.clear();
          router.push('/login');
          return;
        }
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4 flex-1 overflow-hidden">
          ...
          <div className="space-y-1 overflow-auto md:order-1 order-1" style={{ maxHeight: 'calc(85vh - 100px)' }}>
            {/* File Selection Card */}
            <div className={`rounded-xl shadow-sm border p-2 transition-all ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`} id="file-upload">
              <div className="flex items-center mb-5">
                <i className="fas fa-file-upload text-blue-500 mr-3 text-xl"></i>
                <h3 className={`text-md font-bold ${theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}`}>File Selection</h3>
              </div>
              <div className="space-y-2">
                <div className="flex gap-3">
                  <input
                    ...
                    className={`flex-1 px-4 py-1 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition-all ${theme === 'dark' ? 'bg-slate-700 border-slate-600 text-slate-100 placeholder-slate-400' : 'bg-white border-slate-300 text-black placeholder-slate-500'}`}
                  />
                  <button
                    onClick={() => browseFile(currentTab)}
                    className="bg-blue-600 text-white px-1 py-1 rounded-lg hover:bg-blue-700 transition-all duration-200 shadow-md hover:shadow-lg"
                  >
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 bg-green-600 text-white px-1 py-1 rounded-lg hover:bg-green-700 transition-all duration-200 shadow-md hover:shadow-lg"
                  >
            ...
            {/* Tool Selection Card */}
            <div className={`rounded-xl shadow-sm border p-2 transition-all ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`} id="tool-config-section">
              <div className="flex items-center mb-2">
                <i className="fas fa-tools text-blue-500 mr-3 text-xl"></i>
                <h3 className={`text-md font-bold ${theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}`}>Tool Configuration</h3>
              </div>
              <div className="space-y-2">
                <div id="tool-select">
                  <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-gray-700'}`}>Security Tool:</label>
                  <select
                    ...
                    className={`w-full px-1 py-1 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition-all ${theme === 'dark' ? 'bg-slate-700 border-slate-600 text-slate-100' : 'bg-white border-slate-300 text-black'}`}
                  >
                ...
                <button
                  ...
                  className="w-full bg-orange-600 text-white px-1 py-1 rounded-lg hover:bg-orange-700 transition-all duration-200 shadow-md hover:shadow-lg"
                >
                ...
                {currentTab === 'c' && cTool === 'Condition Satisfiability Analysis' && (
                  <div className={`p-1 rounded-lg border transition-all ${theme === 'dark' ? 'bg-slate-700 border-slate-600' : 'bg-blue-50 border-blue-200'}`} id="tool-params">
                    <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-gray-700'}`}>Unwind Bound:</label>
                    <input
                      type="number"
                      value={cbmcBound}
                      onChange={(e) => setCbmcBound(e.target.value)}
                      placeholder="Enter unwind bound value"
                      className={`w-full px-1 py-1 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition-all ${theme === 'dark' ? 'bg-slate-600 border-slate-500 text-slate-100 placeholder-slate-400' : 'bg-white border-slate-300 text-black placeholder-slate-500'}`}
                    />
                  </div>
                )}

                {currentTab === 'c' && cTool === 'DSE based Mutation Analyser' && (
                  <div className={`p-4 rounded-lg border transition-all ${theme === 'dark' ? 'bg-slate-700 border-slate-600' : 'bg-blue-50 border-blue-200'}`}>
                    <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-gray-700'}`}>Tool Value:</label>
                    <select
                      value={kleemaValue}
                      onChange={(e) => setKleemaValue(e.target.value)}
                      className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition-all ${theme === 'dark' ? 'bg-slate-600 border-slate-500 text-slate-100' : 'bg-white border-slate-300 text-black'}`}
                    >
                ...
                {currentTab === 'c' && (cTool === 'Advance Code Coverage Profiler' || cTool === 'Mutation Testing Profiler') && (
                  <div className={`p-4 rounded-lg border space-y-3 transition-all ${theme === 'dark' ? 'bg-slate-700 border-slate-600' : 'bg-blue-50 border-blue-200'}`}>
                    <div>
                      <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-gray-700'}`}>Version:</label>
                      <select
                    ...
                    <div>
                      <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-gray-700'}`}>Time Bound (seconds):</label>
                      <input
                        type="number"
                        ...
                        className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition-all ${theme === 'dark' ? 'bg-slate-600 border-slate-500 text-slate-100 placeholder-slate-400' : 'bg-white border-slate-300 text-black placeholder-slate-500'}`}
                      />
                    </div>
                  </div>
                )}
                ...
                {currentTab === 'solidity' && solidityTool === 'VeriSol' && (
                  <div className={`p-4 rounded-lg border transition-all ${theme === 'dark' ? 'bg-slate-700 border-slate-600' : 'bg-blue-50 border-blue-200'}`}>
                    <label className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-gray-700'}`}>Verification Mode:</label>
                    <select
                      value={solidityMode}
          ...
          <div className={`rounded-xl shadow-sm border overflow-scroll flex flex-col transition-all md:order-2 order-2 ${theme === 'dark' ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`} id="terminal-output" style={{ height: 'calc(96vh - 100px)' }}>
            <div className={`p-3 md:p-4 border-b flex justify-between items-center flex-shrink-0 flex-col md:flex-row gap-3 md:gap-0 ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
              ...
              <div className="flex space-x-2 md:space-x-3 w-full md:w-auto flex-wrap md:flex-nowrap">
                <button
                  ...
                  className="flex-1 md:flex-initial bg-blue-600 text-white px-2 md:px-4 py-1 md:py-2 rounded-lg hover:bg-blue-700 transition-all duration-200 shadow-md hover:shadow-lg text-xs md:text-sm"
                >
            ...
            <div className="p-6">
              <div className={`rounded-lg border shadow-inner overflow-hidden transition-all ${theme === 'dark' ? 'bg-slate-950 border-slate-700' : 'bg-slate-50 border-slate-300'}`} style={{ height: '700px' }}>
                <div className="p-4 h-full overflow-y-auto">
                  <pre className={`font-mono text-sm whitespace-pre-wrap leading-relaxed ${theme === 'dark' ? 'text-green-400' : 'text-green-700'}`}>
